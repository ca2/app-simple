...
#define NANOUI_VERSION                                                          \
 ...
 //#define NANOUI_SHOW_WIDGET_BOUNDS 1
     ...
     *
     * The macro ``NAMESPACE_BEGIN(nanoui)`` will expand to ``namespace
...
#define NAMESPACE_BEGIN(name) namespace name {
      ...
      * ``NAMESPACE_END(nanoui)`` will expand to only ``}``.
      *
      * \param name
 ...
 */
//extern NANOUI_EXPORT void async(const std::function<void()> & func);
 ...
 */
//extern NANOUI_EXPORT std::string
 ...
 */
//extern NANOUI_EXPORT std::vector<std::string>
//file_dialog(const std::vector<std::pair<std::string, std::string>> & filetypes,
 ...
 */
//extern NANOUI_EXPORT std::string utf8(uint32_t c);
//
///// Load a directory of PNG images and upload them to the GPU (suitable for use with ImagePanel)
//extern NANOUI_EXPORT std::vector<std::pair<int, std::string>>
...
//#define nvgImageIcon(ctx, name) nanoui::__nanoui_get_image(ctx, #name, name##_png, name##_png_size)
///// Helper function used by nvg_image_icon
//extern NANOUI_EXPORT int __nanoui_get_image(NVGcontext * ctx, const std::string & name,
