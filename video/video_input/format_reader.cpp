...
HRESULT SpecialCaseAttributeValueNew(GUID guid, const prop_variant& var, ::video_input::media &out);

::u32 *GetParametr(GUID guid, ::video_input::media &out)
{
	if(guid == MF_MT_YUV_MATRIX) 
		return &(out.m_uYuvMatrix);

	if(guid == MF_MT_VIDEO_LIGHTING)
		return &(out.m_uVideoLighting);
		
	if(guid == MF_MT_DEFAULT_STRIDE)
		return &(out.m_uDefaultStride);
		
	if(guid == MF_MT_VIDEO_CHROMA_SITING)
		return &(out.m_uVideoChromaSiting);
	
	if(guid == MF_MT_VIDEO_NOMINAL_RANGE)
		return &(out.m_uVideoNominalRange);

	if(guid == MF_MT_ALL_SAMPLES_INDEPENDENT) 
		return &(out.m_uAllSamplesIndependent);
	
	if(guid == MF_MT_FIXED_SIZE_SAMPLES) 
		return &(out.m_uFixedSizeSamples);

	if(guid == MF_MT_SAMPLE_SIZE) 
		return &(out.m_uSampleSize);

	if(guid == MF_MT_VIDEO_PRIMARIES) 
		return &(out.m_uVideoPrimaries);

	if(guid == MF_MT_INTERLACE_MODE) 
		return &(out.m_uInterlaceMode);

	return NULL;
}


HRESULT LogAttributeValueByIndexNew(IMFAttributes *pAttr, DWORD index, ::video_input::media &out)
{
        ...
        case VT_CLSID:
            ...
            strValName = GetGUIDNameNew(*var.puuid);
				...
				if (strValName.has_char())
				{
					...
					out.m_guidAmFormatType = MF_MT_AM_FORMAT_TYPE;

					out.m_strAmFormatTypeName = strValName;
            ...
            strValName = GetGUIDNameNew(*var.puuid);
				...
				if (strValName.has_char())
				{
					...
					out.m_guidMajorType = MF_MT_MAJOR_TYPE;

					out.m_strMajorTypeName = strValName;
            ...
            strValName = GetGUIDNameNew(*var.puuid);
				...
				if (strValName.has_char())
				{
					...
					out.m_guidSubtype = MF_MT_SUBTYPE;

					out.m_strSubtypeName = strValName;
...
HRESULT SpecialCaseAttributeValueNew(GUID guid, const prop_variant& var, video_input::media &out)
{
	...
	if (guid == MF_MT_FRAME_RATE)
	{
		...
		out.m_uFrameRate = uHigh;
		
		out.m_uFrameRateLow = uLow;
	}
	else
	if (guid == MF_MT_FRAME_RATE_RANGE_MAX)
	{
		...
		out.m_uFrameRateRangeMax = uHigh;
		
		out.m_uFrameRateRangeMaxLow = uLow;
	}
	else
	if (guid == MF_MT_FRAME_RATE_RANGE_MIN)
	{
		...
		out.m_uFrameRateRangeMin = uHigh;
		
		out.m_uFrameRateRangeMinLow = uLow;
	}
	else
	if (guid == MF_MT_PIXEL_ASPECT_RATIO)
	{
		...
		out.m_uPixelAspectRatio = uHigh;
		
		out.m_uPixelAspectRatioLow = uLow;
	}	     
    else
    {
        return S_FALSE;
...
namespace video_input
{
   ...
   media format_reader::Read(IMFMediaType * pmediatype)
   {
      UINT32 m_cCount = 0;
      ...
      {
         hr = LogAttributeValueByIndexNew(pmediatype, i, out);
