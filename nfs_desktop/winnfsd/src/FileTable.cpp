...
FILE_ITEM * GetFileItem(const char * path)
{

   auto pnode = g_FileTree.FindFileItemForPath(path);
...
{
   auto strPath = NfsProcessPath(path);
    return g_FileTable.GetHandleByPath(strPath);
}
...
int RenameFile(const char *pathFrom, const char *pathTo)
{
    ...
    auto strPathFrom = NfsProcessPath(pathFrom);
	node = g_FileTable.FindItemByPath(strPathFrom);
    pItem = &(node->data);

    if (pItem == NULL) {
        return false;
    }
    auto strPathTo = NfsProcessPath(pathTo);
    errno_t errorNumber = rename(strPathFrom, strPathTo);

    if (errorNumber == 0) { //RESULT_SUCCESS
		g_FileTable.RenameFile(strPathFrom, strPathTo);
        return errorNumber;
    }
    else {
        return errorNumber;
...
{
   auto strPath = NfsProcessPath(path);
    g_FileTable.RemoveItem(strPath);
}
