...
static ::u32 __stdcall NFS3ProcCleanupThreadProc(void * lpParameter)
{
   CNFS3Prog * pProg;

   pProg = (CNFS3Prog *)lpParameter;
   pProg->RunCleanup();
...
CNFS3Prog::CNFS3Prog(nfs * pnfs) : CRPCProg(pnfs)
{
    m_nUID = m_nGID = 0;

    m_pmutexCleanup = m_pnfs->node()->create_mutex();
    m_hThreadCleanup = (HANDLE)_beginthreadex(NULL, 0, NFS3ProcCleanupThreadProc, this, 0, &m_idThreadCleanup);  //begin thread
}
...
void CNFS3Prog::SetUserID(::u32 nUID, ::u32 nGID)
{
    m_nUID = nUID;
    m_nGID = nGID;
}
   ...
   while (true) 
   {
...

      {
         ...
         for (int i = 0; i < m_fileitemaCleanup.size(); )
         {
            ...
            auto pItem = m_fileitemaCleanup[i];

            if (pItem == NULL)
            {
               m_fileitemaCleanup.erase_at(i);

               continue;
            }

            if (pItem->m_timeLastFile.elapsed() > 1_s)
            {

               if (pItem->m_pLastFile)
               {

                  fclose(pItem->m_pLastFile);

                  pItem->m_pLastFile = nullptr;
...
int CNFS3Prog::Process(IInputStream *pInStream, IOutputStream *pOutStream, ProcessParam *pParam)
{
    static PPROC pf[] = { 
        ...
        &CNFS3Prog::ProcedureREAD, &CNFS3Prog::ProcedureWRITE1, &CNFS3Prog::ProcedureCREATE, 
   ...
   ::file::path path;
    ...
    if (stat == NFS3_OK) {
        ...
        if (new_attributes.mtime.set_it == SET_TO_SERVER_TIME || new_attributes.atime.set_it == SET_TO_SERVER_TIME){
            hFile = CreateFile(wstring(cStr), FILE_WRITE_ATTRIBUTES, FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);
...
nfsstat3 CNFS3Prog::ProcedureLOOKUP(void)
{
    ...
    stat = CheckFile((::i8*)dirName.c_str(), path);
    ...
    dir_attributes.attributes_follow = GetFileAttributesForNFS((::i8*)dirName.c_str(), &dir_attributes.attributes);
...
nfsstat3 CNFS3Prog::ProcedureREADLINK(void)
{
    ...
    if (stat == NFS3_OK) {
        ...
        hFile = CreateFile(wstring(cStr), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_REPARSE_POINT | FILE_FLAG_OPEN_REPARSE_POINT | FILE_FLAG_BACKUP_SEMANTICS, NULL);
                ...
                if (lpOutBuffer->ReparseTag == IO_REPARSE_TAG_SYMLINK || lpOutBuffer->ReparseTag == IO_REPARSE_TAG_MOUNT_POINT)
                {
                    if (lpOutBuffer->ReparseTag == IO_REPARSE_TAG_SYMLINK)
                    {
                        ...
                        if (!PathIsRelativeA(cPrintName.c_str()))
                        {
                            ...
                            ::i8 *target = _strdup(strFromChar.c_str());
                            // remove last folder
                            auto lastFolderIndex = path.rear_find_index('\\');
                            if (lastFolderIndex>= 0) {
                                path = path.substr(0, lastFolderIndex);
                            }
                            ::i8 szOut[MAX_PATH] = "";
                            PathRelativePathToA(szOut, cStr, FILE_ATTRIBUTE_DIRECTORY, target, FILE_ATTRIBUTE_DIRECTORY);
                    ...
                    if (lpOutBuffer->ReparseTag == IO_REPARSE_TAG_MOUNT_POINT)
                    {
                        ...
                        ::i8 szOut[MAX_PATH] = "";
                        PathRelativePathToA(szOut, cStr, FILE_ATTRIBUTE_DIRECTORY, target.c_str(), FILE_ATTRIBUTE_DIRECTORY);
                        ::string symlinkPath = szOut;
                        finalSymlinkPath.assign(symlinkPath);
                    }
                    ...
                    //::i8 *result = _strdup(finalSymlinkPath.c_str());
                    ::file::path result(finalSymlinkPath);
                    data.Set(result);
                }
                free(lpOutBuffer);
            }
   ...
   ::file::path path;
    ...
    if (stat == NFS3_OK) {
        ...
        if (pFile != NULL) {
            ...
            count = (count3) fread(data.contents, sizeof(::i8), count, pFile);
            eof = fgetc(pFile) == EOF;
            fclose(pFile);
        } else {
...
nfsstat3 CNFS3Prog::ProcedureWRITE1(void)
{
   ::file::path path;
   offset3 offset;
   count3 count;
   ...
   PrintLog(strExtraLog);
    ...
    if (stat == NFS3_OK) {
       ...
       if (!pfileitem->m_t.m_pLastFile)
       {
          ...
          pfileitem->m_t.m_pLastFile = _fsopen(cStr, "r+b", _SH_DENYWR);

          m_fileitemaCleanup.add(&pfileitem->m_t);

       }

       pfileitem->m_t.m_timeLastFile.Now();

       pFile = pfileitem->m_t.m_pLastFile;
        ...
        if (stable == UNSTABLE) {
            ...
            ////if (unstableStorageFile1.count(handleId) == 0){
            //   if (!pfileitem->m_t.m_pLastFile)
            //   {
            ...
            //   pFile = pfileitem->m_t.m_pLastFile;
            //    //if (pFile != NULL) {
            //      // unstableStorageFile1.insert(std::make_pair(handleId, pFile));
            //    //}
            ////} else {
            ////    pFile = unstableStorageFile1[handleId];
            ////}

            file_wcc.before.attributes_follow = GetFileAttributesForNFS(pFile, cStr, &file_wcc.before.attributes);

            if (pFile != NULL) {
                _fseeki64(pFile, offset, SEEK_SET);
                count = (count3) fwrite(data.contents, sizeof(::i8), data.length, pFile);
            } else {
                ::i8 buffer[BUFFER_SIZE];
                errno_t errorNumber = errno;
                strerror_s(buffer, BUFFER_SIZE, errorNumber);
            ...
            if (pFile != NULL) {
               file_wcc.before.attributes_follow = GetFileAttributesForNFS(pFile, cStr, &file_wcc.before.attributes);
                _fseeki64(pFile, offset, SEEK_SET) ;
                ::memory m;
                m.set_size(data.length);
                fread(m.data(), sizeof(::i8), data.length, pFile);
                if (memcmp(m.data(), data.contents, data.length) == 0)
                {
                   PrintLog(" ok ");
                }
                else
...
nfsstat3 CNFS3Prog::ProcedureCREATE(void)
{
    ...
    dir_wcc.before.attributes_follow = GetFileAttributesForNFS((::i8*)dirName.c_str(), &dir_wcc.before.attributes);
    ...
    dir_wcc.after.attributes_follow = GetFileAttributesForNFS((::i8*)dirName.c_str(), &dir_wcc.after.attributes);
...
nfsstat3 CNFS3Prog::ProcedureSYMLINK(void)
{
    ...
    _In_ LPSTR lpSymlinkFileName = path; // symlink (full path)
    ...
    _In_ LPSTR fullTargetPathString = const_cast<LPSTR>(fullTargetPath.c_str());
    GetFullPathNameA(fullTargetPathString, MAX_PATH, fullTargetPathNormalized, NULL);
    targetFileAttr = GetFileAttributesA(fullTargetPathNormalized);
    ...
    BOOLEAN failed = CreateSymbolicLinkW(wstring(lpSymlinkFileName), wstring(lpTargetFileName), dwFlags);
...
nfsstat3 CNFS3Prog::ProcedureRENAME(void)
{
    ...
    stat = CheckFile((::i8*)dirFromName.c_str(), pathFrom);

    fromdir_wcc.before.attributes_follow = GetFileAttributesForNFS((::i8*)dirFromName.c_str(), &fromdir_wcc.before.attributes);
    todir_wcc.before.attributes_follow = GetFileAttributesForNFS((::i8*)dirToName.c_str(), &todir_wcc.before.attributes);
    
    if (FileExists(pathTo)) {
		DWORD fileAttr = GetFileAttributesA(pathTo);
		if ((fileAttr & FILE_ATTRIBUTE_DIRECTORY) && (fileAttr & FILE_ATTRIBUTE_REPARSE_POINT)) {
			returnCode = EraseFolder(pathTo);
			if (returnCode != 0) {
				if (returnCode == ERROR_DIR_NOT_EMPTY) {
					stat = NFS3ERR_NOTEMPTY;
				} else {
    ...
    fromdir_wcc.after.attributes_follow = GetFileAttributesForNFS((::i8*)dirFromName.c_str(), &fromdir_wcc.after.attributes);
    todir_wcc.after.attributes_follow = GetFileAttributesForNFS((::i8*)dirToName.c_str(), &todir_wcc.after.attributes);
...
nfsstat3 CNFS3Prog::ProcedureLINK(void)
{
    ...
    ::i8 *linkFullPath = GetFullPath(dirName, fileName);

    //TODO: Improve checks here, cStr may be NULL because handle is invalid
    if (CreateHardLinkA(linkFullPath, cStr, NULL) == 0) {
        stat = NFS3ERR_IO;
    }
...
nfsstat3 CNFS3Prog::ProcedureREADDIR(void)
{
    ...
    if (stat == NFS3_OK) {
        ...
        ::wstring wstrFilePath(filePath);
        eof = true;
        handle = _wfindfirst(wstrFilePath, &fileinfo);
        bFollows = true;

        if (handle) {
            nFound = 0;

            for (i = (::u32)cookie; i > 0; i--) {
                nFound = _wfindnext(handle, &fileinfo);
            }
            ...
            if (nFound == 0) {
                ...
                do {
                    ...
                    sprintf_s(filePath, "%s\\%s", cStr, strName.c_str());
                    fileid = get_file_handle(filePath).m_ll;
...
nfsstat3 CNFS3Prog::ProcedureREADDIRPLUS(void)
{
    ...
    if (stat == NFS3_OK) {
        Write(&cookieverf);
        sprintf_s(filePath, "%s\\*", cStr);
        handle = _wfindfirst(::wstring(filePath), &fileinfo);
        eof = true;
...
nfsstat3 CNFS3Prog::ProcedureFSSTAT(void)
{
    ...
    if (stat == NFS3_OK) {
        ...
        if (obj_attributes.attributes_follow
            && GetDiskFreeSpaceExA(cStr, (PULARGE_INTEGER)&fbytes, (PULARGE_INTEGER)&tbytes, (PULARGE_INTEGER)&abytes)
            ) {
            //tfiles = 99999999999;
            //ffiles = 99999999999;
            //afiles = 99999999999;
...
nfsstat3 CNFS3Prog::ProcedureCOMMIT(void)
{
...

    {
       ...
       if (pfileitem->m_t.m_pLastFile)
       {
          ...
          fflush(pfileitem->m_t.m_pLastFile);

          stat = NFS3_OK;
       }
       else
       //}
       //else if (unstableStorageFile[handleId] != NULL) 
       //{
       //   m_pnfs->file()->copy(cStr, unstableStorageFile[handleId]);
...
bool CNFS3Prog::GetPath(::file::path &path)
{
    nfs_fh3 object;

    Read(&object);
     path = GetFilePath(object);
     bool valid = path.has_character();
    if (valid) {
        PrintLog(" %s ", path.c_str());
    } else {
...
bool CNFS3Prog::ReadDirectory(::file::path & dirName, ::string &fileName)
{
    diropargs3 fileRequest;
    Read(&fileRequest);

    dirName = GetFilePath(fileRequest.dir);

    if (dirName.has_character()) {
        fileName = ::string(fileRequest.name.name);
        return true;
    } else {
        return false;
    }
...
::i8 *CNFS3Prog::GetFullPath(::string &dirName, ::string &fileName)
{
    //TODO: Return ::string
    static ::i8 fullPath[MAXPATHLEN + 1];
...
nfsstat3 CNFS3Prog::CheckFile(const_char_pointer fullPath)
{
   ...
   auto strFullPath = NfsProcessPath(fullPath);
...
bool CNFS3Prog::GetFileHandle(const_char_pointer path, nfs_fh3 *pObject)
{
   ...
   auto filehandle = CFileTableAccessor::get_file_handle(path);
    ...
    auto err = memcpy_s(pObject->contents, NFS3_FHSIZE, &filehandle.m_ll, pObject->length);
	if (err != 0) {
		PrintLog(" err %d ", err);
		return false;
	}
...
bool CNFS3Prog::GetFileAttributesForNFS(const_char_pointer path, wcc_attr *pAttr)
{
    ...
    pAttr->mtime.seconds = (::u32)data.st_mtime;
    ...
    pAttr->ctime.seconds = (::u32)data.st_mtime;
...
bool CNFS3Prog::GetUnstableFileAttributesForNFS(::memory_file * pfile, const_char_pointer path, fattr3 * pAttr)
{
   ...
   //fileAttr = GetFileAttributesW(wstrPath);
...
//   }
   ...
   //hFile = CreateFileW(wstrPath, FILE_READ_EA, FILE_SHARE_READ, NULL, OPEN_EXISTING, dwFlagsAndAttributes, NULL);
...
bool CNFS3Prog::GetFileAttributesForNFS(FILE * pfile, tree_path_item<FILE_ITEM> * pfileitem, const_char_pointer path, fattr3 * pAttr)
{
   ...
   class ::time timeNow(now_t{});

   if ((timeNow - pfileitem->m_t.m_timeLastFileAttribute) > 1_s)
   {

      pfileitem->m_t.m_uLastFileAttributes = GetFileAttributesW(wstrPath);

      pfileitem->m_t.m_timeLastFileAttribute = timeNow;

   }

   fileAttr = pfileitem->m_t.m_uLastFileAttributes;
