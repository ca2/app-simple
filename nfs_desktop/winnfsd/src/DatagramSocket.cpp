...
CDatagramSocket::CDatagramSocket(nfs * pnfs):
   m_pnfs(pnfs)
{
    m_nPort = 0;
    m_bClosed = true;
...
bool CDatagramSocket::Open(int nPort)
{
	...
	localAddr.sin_addr.s_addr = inet_addr(m_pnfs->get_bind_address());
	if (localAddr.sin_addr.s_addr == INADDR_NONE) {
      m_pnfs->set_bind_address("0.0.0.0");
		//m_pnfs->m_sInAddr = "0.0.0.0";
