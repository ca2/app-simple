...
public:
   ...
   tree_path_item(tree_path_item * pitemParent, const ::scoped_string & scopedstr) :
   ...
   tree_path_item<T> * m_pitemParent;
   ::comparable_array < tree_path_item < T > * >  m_itema;
   T m_t;

   tree_path_item<T> * root()
   {
      if (m_pitemParent)
      {

         return m_pitemParent->root();
   ...
   virtual tree_path_item <T> * create_new_tree_path_item(tree_path_item<T > * pitemParent, const ::scoped_string & scopedstr)
   {
   ...
   ::file::path path()
   {
      ...
      if (m_pitemParent)
      {
         ...
         return m_pitemParent->path() / m_t.name();
   ...
   tree_path_item<T> * find_child(const ::scoped_string & scopedstr)
   {

      for (auto & pitem : m_itema)
      {

         if (pitem->m_t.name() == scopedstr)
         {
   ...
   tree_path_item<T> * get_child(const ::scoped_string & scopedstr)
   {

      auto pitem = find_child(scopedstr);

      if (!pitem)
      {

         pitem = root()->create_new_tree_path_item(this, scopedstr);
   ...
   tree_path_item * get_item(const ::scoped_string & scopedstr)
   {
   ...
   tree_path_item * find_item(const ::scoped_string & scopedstr)
   {
   ...
   void erase_child(tree_path_item * pitem)
   {
   ...
   void __rename_item(const ::scoped_string & scopedstrTo,
      const ::scoped_string & scopedstrFrom)
   {

      erase_item_by_path(scopedstrFrom);
      get_item(scopedstrTo);

   }
   void erase_item(tree_path_item * pitem)
   {

      auto path = pitem->path();

      erase_item_by_path(path);

   }
   void erase_item_by_path(const ::scoped_string & scopedstr)
   {
      auto pitem = find_item(scopedstr);
...
//template<class T>
//tree_path_item<T>::tree_path_item()
//	: parent(0), first_child(0), last_child(0), prev_sibling(0), next_sibling(0)
//	{
//	}
//
//template<class T>
//tree_path_item<T>::tree_path_item(const T& val)
...
//class tree {
//	protected:
//		typedef tree_path_item<T> tree_node;
//	public:
//      tree_node * m_pnodeHead, * m_pnodeTail;    // m_pnodeHead/m_pnodeTail are always dummy; if an iterator points to them it is invalid
//
//
...
//   { 
//   m_pnodeHead = new tree_path_item<T>(); // MSVC does not have default second argument 
//	m_pnodeTail = new tree_path_item<T>();
//
//   m_pnodeHead->parent=0;
//   m_pnodeHead->first_child=0;
//   m_pnodeHead->last_child=0;
//   m_pnodeHead->prev_sibling=0; //m_pnodeHead;
//   m_pnodeHead->next_sibling=m_pnodeTail; //m_pnodeHead;
//
//	m_pnodeTail->parent=0;
//	m_pnodeTail->first_child=0;
//	m_pnodeTail->last_child=0;
//	m_pnodeTail->prev_sibling=m_pnodeHead;
//	m_pnodeTail->next_sibling=0;
//   }
//
//template <class T>
...
//	{
//	if(m_pnodeHead)
//		while(m_pnodeHead->next_sibling!=m_pnodeTail)
//			erase(pre_order_iterator(m_pnodeHead->next_sibling));
//	}
//
//template<class T> 
...
//	{
//	return pre_order_iterator(m_pnodeHead->next_sibling);
//	}
//
//template <class T>
//typename tree<T>::pre_order_iterator tree<T>::end() const
//	{
//	return pre_order_iterator(m_pnodeTail);
//	}
//
//template <class T>
//typename tree<T>::breadth_first_queued_iterator tree<T>::begin_breadth_first() const
//	{
//	return breadth_first_queued_iterator(m_pnodeHead->next_sibling);
//	}
//
//template <class T>
...
//	{
//	tree_node *tmp=m_pnodeHead->next_sibling;
//	if(tmp!=m_pnodeTail) {
...
//	{
//	return post_order_iterator(m_pnodeTail);
//	}
//
//template <class T>
...
//   {
//   return leaf_iterator(m_pnodeTail);
//   }
//
//template <class T>
...
// 	{
//	assert(position.node!=m_pnodeHead);
//	assert(position.node!=m_pnodeTail);
//	assert(position.node);
//
//	tree_node *tmp=new tree_path_item<T>();
...
//	assert(position.node);
//
//	tree_node* tmp = new tree_path_item <T>(x);
...
//template <class T>
//template <class iter>
//iter tree<T>::append_children(iter position, sibling_iterator from, sibling_iterator to)
//	{
//	assert(position.node!=m_pnodeHead);
//	assert(position.node!=m_pnodeTail);
//	assert(position.node);
...
//template <class T>
//template <class iter>
//iter tree<T>::prepend_children(iter position, sibling_iterator from, sibling_iterator to)
//	{
//	assert(position.node!=m_pnodeHead);
//	assert(position.node!=m_pnodeTail);
//	assert(position.node);
...
//	{
//	assert(m_pnodeHead->next_sibling==m_pnodeTail);
//	return insert(iterator(m_pnodeTail), x);
//	}
//
//template <class T>
//template <class iter>
...
//	{
//	if(position.node==0) {
//		position.node=m_pnodeTail; // Backward compatibility: when calling insert on a null node,
//		                    // insert before the m_pnodeTail.
//		}
//	//tree_node* tmp = alloc_.allocate(1,0);
//	//alloc_.construct(tmp, x);
//   auto tmp = new tree_path_item<T>(x);
////	kp::constructor(&tmp->data, x);
//	tmp->first_child=0;
//	tmp->last_child=0;
//
...
//		if(tmp->parent) // when inserting nodes at the m_pnodeHead, there is no parent
...
//template <class T>
//template <class iter>
//iter tree<T>::insert_subtree(iter position, const iterator_base& subtree)
//	{
//	// insert dummy
...
//template <class T>
//template <class iter>
//iter tree<T>::insert_subtree_after(iter position, const iterator_base& subtree)
//	{
//	// insert dummy
...
//// template <class T>
//// template <class iter>
//// iter tree<T>::insert_subtree(sibling_iterator position, iter subtree)
//// 	{
//// 	// insert dummy
//// 	iter it(insert(position, value_type()));
//// 	// replace dummy with subtree
...
//
//	// replace the node at position with m_pnodeHead of the replacement tree at from
////	std::cout << "warning!" << position.node << std::endl;
//	erase_children(position);	
////	std::cout << "no warning!" << std::endl;
//   auto tmp = new tree_path_item<T>(*from);
//	//tree_node* tmp = alloc_.allocate(1,0);
...
//template <class T>
//template <typename iter>
//iter tree<T>::reparent(iter position, sibling_iterator begin, sibling_iterator end)
//	{
...
//template <class T>
//typename tree<T>::sibling_iterator tree<T>::move_before(sibling_iterator target, 
...
//	if(prev==0) {
//		if((*nit)->parent!=0) // to catch "sorting the m_pnodeHead" situations, when there is no parent
...
//	if(next==0) {
//		if((*eit)->parent!=0) // to catch "sorting the m_pnodeHead" situations, when there is no parent
...
//template <class T>
//template <typename iter, class BinaryPredicate>
//bool tree<T>::equal(const iter& one_, const iter& two, const iter& three_, BinaryPredicate fun) const
//	{
...
//template <class T>
//template <typename iter, class BinaryPredicate>
//bool tree<T>::equal_subtree(const iter& one_, const iter& two_, BinaryPredicate fun) const
//	{
...
//
//template <class T>
//void tree<T>::subtree(tree& tmp, sibling_iterator from, sibling_iterator to) const
//	{
//	tmp.set_head(value_type());
//	tmp.replace(tmp.begin(), tmp.end(), from, to);
//	}
...
//	{
//	int maxd=-1;
//	for(tree_node *it = m_pnodeHead->next_sibling; it!=m_pnodeTail; it=it->next_sibling)
...
//	tree_node *tmp=pos.node;
//
//	if(tmp==0 || tmp==m_pnodeHead || tmp==m_pnodeTail) return -1;
//
...
//			pos->prev_sibling!=m_pnodeHead &&
//			pos->prev_sibling!=m_pnodeTail) {
//		++ret;
...
//
//template <class T>
//bool tree<T>::is_in_subtree(const iterator_base& it, const iterator_base& begin, 
//																 const iterator_base& end) const
//	{
...
//	{
//	if(it.node==0 || it.node==m_pnodeTail || it.node==m_pnodeHead) return false;
//	else return true;
//	}
//
//template <class T>
...
//
//template <class T>
//tree<T>::breadth_first_queued_iterator::breadth_first_queued_iterator(tree_node *tn)
//	: iterator_base(tn)
//	{
...
//
//template <class T>
//tree<T>::breadth_first_queued_iterator::breadth_first_queued_iterator(const iterator_base& other)
//	: iterator_base(other.node)
//	{
...
//
//template <class T>
//bool tree<T>::breadth_first_queued_iterator::operator!=(const breadth_first_queued_iterator& other) const
//	{
//	if(other.node!=this->node) return true;
//	else return false;
//	}
...
//
//template <class T>
//typename tree<T>::breadth_first_queued_iterator tree<T>::breadth_first_queued_iterator::operator++(int)
//	{
...
//
//template <class T>
//tree<T>::fixed_depth_iterator::fixed_depth_iterator(const sibling_iterator& other)
//	: iterator_base(other.node), top_node(0)
//	{
//	}
//
//template <class T>
//tree<T>::fixed_depth_iterator::fixed_depth_iterator(const fixed_depth_iterator& other)
//	: iterator_base(other.node), top_node(other.top_node)
//	{
//	}
//
//template <class T>
//bool tree<T>::fixed_depth_iterator::operator==(const fixed_depth_iterator& other) const
//	{
...
////		this->node=this->node->prev_sibling;
////		assert(this->node!=0);
////		if(this->node->parent==0 && this->node->prev_sibling==0) // m_pnodeHead element
////			this->node=0;
////		}
////	else {
...
//
//template <class T>
//typename tree<T>::fixed_depth_iterator tree<T>::fixed_depth_iterator::operator++(int)
//	{
