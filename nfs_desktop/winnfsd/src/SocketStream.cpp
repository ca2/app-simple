...
{
    m_pInBuffer = new ::u8[MAXDATA];
    m_pOutBuffer = new ::u8[MAXDATA];
...
::u32 CSocketStream::Read(unsigned long *pnValue)
{
    ::u32 i, n;
    ::u8 *p, pBuffer[sizeof(unsigned long)];
...
::u32 CSocketStream::Read8(unsigned __int64 *pnValue)
{
    ::u32 i, n;
    ::u8 *p, pBuffer[sizeof(unsigned __int64)];
...
::u32 CSocketStream::Skip(::u32 nSize)
{
    if (nSize > m_nInBufferSize - m_nInBufferIndex) { //over the number of bytes of data in the input buffer
        nSize = m_nInBufferSize - m_nInBufferIndex;
    }
