...
namespace app_simple_shortcut_amender
{
   ...
   ::i64 form_amender::increment_reference_count()
   {
   ...
   ::i64 form_amender::decrement_reference_count()
   {
   ...
   void form_amender::install_message_routing(::channel * psender)
   {
      ...
      USER_MESSAGE_LINK(::user::e_message_create, psender, this, &form_amender::on_message_create);
   ...
   void form_amender::on_message_create(::message::message * pmessage)
   {
      ...
      construct_newø(m_pstillTarget);

      construct_newø(m_peditTarget);
   ...
   void form_amender::on_layout(::draw2d::graphics_pointer & pdraw2dgraphics)
   {
...

      {
         ...
         m_pstillFolder->display_child(::f64_rectangle_dimension(iLeft, y, sizeStill.cx, sizeStill.cy));
...

      {
         ...
         m_peditFolder->display_child(::f64_rectangle_dimension(iLeft, y, 600, sizeEdit.cy));
...

      {
         ...
         m_pstillSource->display_child(::f64_rectangle_dimension(iLeft, y, sizeStill.cx, sizeStill.cy));
...

      {
         ...
         auto sizeEdit = m_peditSource->get_adjusted_fitting_size(pdraw2dgraphics);

         auto rectangleEditMargin = m_peditSource->get_margin(m_peditSource->get_style(pdraw2dgraphics), ::e_element_none);

         y += rectangleEditMargin.top;

         m_peditSource->display_child(::f64_rectangle_dimension(iLeft, y, 600, sizeEdit.cy));
...

      {
         ...
         auto sizeStill = m_pstillTarget->get_adjusted_fitting_size(pdraw2dgraphics);

         auto rectangleStillMargin = m_pstillTarget->get_margin(m_pstillTarget->get_style(pdraw2dgraphics));

         y += rectangleStillMargin.top;

         m_pstillTarget->display_child(::f64_rectangle_dimension(iLeft, y, sizeStill.cx, sizeStill.cy));
...

      {
         ...
         sizeEdit = m_peditTarget->get_adjusted_fitting_size(pdraw2dgraphics);

         auto rectangleEditMargin = m_peditTarget->get_margin(m_peditTarget->get_style(pdraw2dgraphics), ::e_element_none);

         y += rectangleEditMargin.top;

         m_peditTarget->display_child(::f64_rectangle_dimension(iLeft, y, 600, sizeEdit.cy));
      ...
      auto sizeButtonMarginClear = m_pbuttonClear->get_margin(m_peditTarget->get_style(pdraw2dgraphics));

      auto sizeButtonMarginSend = m_pbuttonSend->get_margin(m_peditTarget->get_style(pdraw2dgraphics));
      ...
      m_pstillReceiver->display_child(::f64_rectangle_dimension(iLeft, y, 4000, sizeEdit.cy * 100));
