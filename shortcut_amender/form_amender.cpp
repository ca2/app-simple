...
//      }
   ...
   void form_amender::_001OnSendButton(::message::message * pmessage)
   {
      ...
      string strAction = do_amender(strTarget, strFolder, strSource, bThumbnail);
   ...
   string form_amender::do_amender(const ::scoped_string & scopedstrTarget, const ::file::path & pathFolder, const ::scoped_string & scopedstrSource, bool bThumbnail)
   {
      ...
      if (listing.is_empty())
      {
         ...
         strAction = "No link files in Folder '" + pathFolder + "'";
      ...
      for (auto & path : listing)
      {
         ...
         auto plinkPatched = plink->path_find_replace(strSource, strTarget);

         auto elinkChanged = plinkPatched->m_elink;
         ...
         ::file::e_link elinkResult = elinkChanged;
         ...
         if (!bThumbnail)
         {
            ...
            try
            {
               ...
               plinkPatched->write(path);

               elinkResult = plinkPatched->m_elink;

            }
            catch (...)
            {

               elinkResult = ::file::e_link_none;
         ...
         if (elinkChanged & ::file::e_link_target)
         {
            ...
            strAction +=
               (elinkResult & ::file::e_link_target ? "" : "✗!! ")
               + path + ": "
               + plinkPatched->m_pathTarget.windows_path();
               + " <== "
         ...
         if (elinkChanged & ::file::e_link_folder)
         {
            ...
            strAction +=
               (elinkResult & ::file::e_link_folder ? "" : "✗!! ")
               + path + ": "
               + plinkPatched->m_pathFolder.windows_path()
               + " <== "
         ...
         if (elinkChanged & ::file::e_link_icon)
         {
            ...
            strAction +=
               (elinkResult & ::file::e_link_icon ? "" : "✗!! ")
               + path + ": "
               + plinkPatched->m_pathIcon.windows_path()
               + " <== "
